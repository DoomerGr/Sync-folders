...
procedure ScanFile(StartDir: string; Mask:string; List:TStrings;Crc:byte);
procedure ScanDisk(StartDir: string; Mask:string; List:TStrings;Fst:word;Crc:byte);
...
// Fst 2 файлы в указанной папке
 ...
 if Fst=2 then begin ScanFile(StartDir,Mask,List,Crc);exit; end;
 if Fst=0 then ScanFile(StartDir,Mask,List,Crc);
 Fst:=Fst+5;
    ...
    repeat
     if (SearchRec.Attr and faDirectory) = faDirectory then
      if (SearchRec.Name <> '..') and (SearchRec.Name <> '.') then
       begin
        if Fst<>6 then ScanFile(StartDir + SearchRec.Name + '\',Mask,List,Crc);
        ScanDisk(StartDir + SearchRec.Name + '\',Mask,List,Fst,Crc);
       end
      else
...
var
  fos: TSHFileOpStruct;
begin
 FmSinhron.AddEchoText(FmSinhron.RzRichEditEchoCom,'Удаление папки: '+dir,clGreen,FmSinhron.Task.SaveLog);
 Result:=false;
  try
   ...
   on Exception do
      FmSinhron.AddEchoText(FmSinhron.RzRichEditEchoCom,'Ошибка удаления папки: '+dir,clRed,FmSinhron.Task.SaveLog);
  end;
end;
